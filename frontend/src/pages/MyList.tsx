...
import { CollegeListTable } from '../components/CollegeListTable';
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const collegeResponse = await fetch(
                `${API_BASE}/api/college-list/detailed`,
                {
                    headers: {
            ...
            const { data: exclusionData, error: exclusionError } =
                await supabase
    ...
    const searchColleges = useCallback(async (query: string) => {
        ...
        try {
            const response = await fetch(
                `${API_BASE}/api/colleges/search?q=${encodeURIComponent(query)}&limit=8`
            );
            if (response.ok) {
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-zinc-400">
                        {colleges.length}{' '}
                        {colleges.length === 1 ? 'school' : 'schools'} saved
                    </p>
                </div>
            </div>
